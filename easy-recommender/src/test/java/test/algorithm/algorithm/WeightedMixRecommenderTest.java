...
public class WeightedMixRecommenderTest {
	...
	public void testDoRecommend() throws Exception {
		...
		UserBasedCFRecommender cfRecommender1 = new UserBasedCFRecommender("job-recommender", mySQLPreferenceData1, "euclideanDistanceSimilarity", 10, null);
		...
		//创建加权型混合推荐器
		Entry<MyRecommender, Float> entry1 = new MyEntry<MyRecommender, Float>(contentBasedRecommender, 0.6f);
		Entry<MyRecommender, Float> entry2 = new MyEntry<MyRecommender, Float>(cfRecommender1, 1.2f);
		Entry<MyRecommender, Float> entry3 = new MyEntry<MyRecommender, Float>(cfRecommender6, 1.0f);
		...
		recommenderList.add(entry3);
			
		WeightedMixRecommender weightedMixRecommender = new WeightedMixRecommender(recommenderList);	
		weightedMixRecommender.doRecommend(21, 20);
